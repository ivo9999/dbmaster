...
export async function GET(
  ...
  try {
    ...
    if (connection.dbType === "clickhouse") {
      const connectionString = `clickhouse://${connection.username}:${encodeURIComponent(decryptedPassword)}@${connection.host}:${connection.port}/${branch.name}`;
      return NextResponse.json({
        connectionString,
    ...
    const connectionString = `postgresql://${connection.username}:${encodeURIComponent(decryptedPassword)}@${connection.host}:${connection.port}/${branch.name}`;
    ...
    if (connection.publicHost) {
      ...
      devConnectionString = `postgresql://${connection.username}:${encodeURIComponent(decryptedPassword)}@${connection.publicHost}:${publicPort}/${branch.name}?sslmode=disable&connect_timeout=10`;
    }
