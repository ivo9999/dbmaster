...
export default async function TablesPage({ params }: Props) {
  ...
  // Check access (admins have access to all, others need explicit access)
  const hasAccess = session.user.role === "ADMIN" || connection.users.length > 0;
  if (!hasAccess) {
    redirect("/");
  }

  // Fetch tables from the target database
  ...
  if (tables.length > 0) {
    ...
    redirect(`/${connectionId}/db/${connection.database}/tables/${tableName}`);
  }
